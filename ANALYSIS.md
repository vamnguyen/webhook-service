...
- ✅ **Dependency Inversion Principle (SOLID)**: Introduced `IWebhookService` interface to decouple Controller from Service implementation.
- ✅ **Strategy Pattern**: Implemented `WebhookStrategyFactory` to handle different event types (`payment.completed`, etc.) with specific logic, adhering to Open/Closed Principle.
